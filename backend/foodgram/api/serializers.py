...
class UserProfileSerializer(
    ...
    def get_is_subscribed(self, obj):
        """Возвращает True, если текущий пользователь подписан на этого пользователя (автора)."""
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            subscriber = request.user
            return Subscribe.objects.filter(subscriber=subscriber, author=obj).exists()
        return False
...
class RecipeSerializer(serializers.ModelSerializer):
    ...
    def validate_ingredients(self, value):
        ...
        for item in value:
            ...
            if id in ids:
                raise serializers.ValidationError(
                    {"ingredients": (
                        "Поле не может содержать повторяющиеся id ингредиентов."
                    )}
                )
            ids.append(id)
        ids = []
    ...
    def get_ingredients(self, obj):
        """Возвращает список ингредиентов с количеством из промежуточной модели."""
...
class SubscribeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if obj_id is None:
            raise serializers.ValidationError({"detail": "Пользователь не указан."})

        # Проверяем, существует ли такой пользователь
        if not User.objects.filter(id=obj_id).exists():
            raise serializers.ValidationError({"detail": "Пользователь не существует."})

        # Проверка, не пытается ли пользователь подписаться на себя
        if self.request.user.id == int(obj_id):
            raise serializers.ValidationError({"detail": "Вы не можете подписаться на себя."})
