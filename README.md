...
```bash
git clone https://github.com/VitsVi/foodgram.git
```
...
```bash
python manage.py add_ingidients_data 
```
...
### Заполните базу тестовыми данными:
```bash
docker-compose exec backend python manage.py add_ingredients_data   
```
...
### Автор:  
_Жигульский Виталий_<br>
**email**: _jigulsky.vitalik@yandex.ru_<br>